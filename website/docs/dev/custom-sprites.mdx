...
:::info
If you can't reference it right after adding it, try adding a simple string resource to the `Resources.resx` using Visual Studio's default resources editor and then deleting it, or renaming an existing resource, saving, and then renaming it back.
:::
