...
Go to `<GameName>_Data/Managed` in the game's root, grab the following files and put them in `Libraries`:
...
Add a `BepInPlugin` attribute to it with your plugin's <abbr title="Globally Unique Identifier">GUID</abbr>, name and version.  
You can also put `string`s right into the attribute, without using `const`s, if you want.
