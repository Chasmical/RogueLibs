...
Put `RogueLibsPatcher.dll` file into `patchers` ***(not `plugins`!)*** directory:
...
:::caution
RogueLibs v2.x.x is outdated and is not compatible with new mods! There's only a couple of old mods that still use it.
:::
