...
function (
  ...
  defaultValue?: string | (() => string),   // default value of the storage slot
...
:::caution
The value is read from the local storage in an `useEffect` hook, so, on the first render, the value will have the default value.
:::
