...
:::caution
Make sure that you're calling `CreateCustomAbility` and not `CreateCustomItem` method. RogueLibs will throw an exception, if you call `CreateCustomItem` with a `CustomAbility` type.
:::
