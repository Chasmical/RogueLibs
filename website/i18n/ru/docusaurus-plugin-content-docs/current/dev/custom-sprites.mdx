...
:::info
Если у вас не получается обращаться к ресурсам после этого, попробуйте добавить простой ресурс-строку в `Resources.resx` используя редактор в Visual Studio по умолчанию и удалить его, или же попробуйте переименовать уже существующий ресурс, сохранить и переименовать его обратно.
:::
